...
func _ready():
	display_options.select(1 if Singleton.game_data.fullscreen_on else 0)
	Global.toggle_fullscreen(Singleton.game_data.fullscreen_on)
	vsync_btn.pressed = Singleton.game_data.vsync_on
	display_fps_btn.pressed = Singleton.game_data.display_fps
	max_fps_slider.value = Singleton.game_data.max_fps
	bloom_btn.pressed = Singleton.game_data.bloom_on
	brightness_slider.value = Singleton.game_data.brightness
	master_slider.value = Singleton.game_data.master_vol
	fov_slider.value = Singleton.game_data.fov
	mouse_slider.value = Singleton.game_data.mouse_sens
...
func _on_BloomBtn_toggled(button_pressed):
	Global.toggle_bloom(button_pressed)
	

func _on_BrightnessSlider_value_changed(value):
	Global.update_brightness(value)
